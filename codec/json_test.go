...
func (suite *JSONSuite) TestDecode_validTypes() {
	...
	var stringValue string
	suite.NoError(decoder.Decode(bytes.NewBufferString(`"Abide"`), &stringValue))
...
func (suite *JSONSuite) TestDecode_remapping() {
	...
	suite.NoError(decoder.Decode(bytes.NewBufferString(`{"ID":"Abide", "Name":"The Dude"}`), &value))
	suite.Equal("Abide", value.ID)
	suite.Equal("", value.Name)

	suite.NoError(decoder.Decode(bytes.NewBufferString(`{"ID":"Abide", "goes_by":"The Dude"}`), &value))
	suite.Equal("Abide", value.ID)
